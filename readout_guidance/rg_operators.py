...
def _run_aggregation(feats, aggregation_network, emb=None):
    feats = einops.rearrange(feats, 'b w h c -> b c w h') #feats.shape = torch.Size([1, 10560, 64, 64])
    feats = aggregation_network(feats, emb)
    feats = einops.rearrange(feats, 'b c w h -> b w h c') 
    return feats #feats.shape = torch.Size([1, 64, 64, 4])
...
def loss_spatial(obs_feat, gt_feat, edit, latents_scale=(0, 1)):
    ...
    # print(f"{obs_feat.shape=}")
    # print(f"{control_feat.shape=}")
    _, _, obs_w, obs_h = obs_feat.shape
    # Normalize from arbitrary output scale
    obs_feat = renormalize(obs_feat, edit["control_range"], latents_scale) #ranges: obs_feat[-1,2] , edit["control_range"][-1,1] ,latents_scale[-4.67,3.991]
    ...
    # control_feat = torch.nn.functional.interpolate(control_feat, (obs_w, obs_h)) #obsfeat went from [-1,2] to [-4.86,8.427]
    # loss = torch.nn.functional.mse_loss(obs_feat, control_feat, reduction="none") #gt_feat went from [-1,2] to [-4.86,8.427]
    difference = obs_feat -control_feat #control feat went from [-11.5,8.731] (actually its a latent so -4,4) to [-50.441,37.47]
...
def loss_guidance(controller, feats, batch_idx, gt_idx, edits=[], log=False, emb=None, latents_scale=None, t=None, i=None):
    ...
    for edit in edits:
        ...
        gt_feat = feats[gt_idx][None, ...].detach().clone() #gt_idx = 0. so we assume the 
        obs_feat = feats[batch_idx][None, ...] #batch_idx = 1
        ...
        if edit["head_type"] == "spatial":
            spa_loss, obs_feat, gt_feat = loss_spatial(obs_feat, gt_feat, edit, latents_scale) #here
