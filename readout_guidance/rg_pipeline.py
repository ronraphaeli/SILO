...
def diffusion_step(
    ...
    def _diffusion_step(latents):
        ...
        noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2) #!TODO: we can ommit all the uncond if cfg = 1
...
def text2image_rg(
    ...
    with torch.no_grad():
        ...
        if "xl" in model_id:
            ...
            context = rg_helpers.get_context(model, prompt, negative_prompt=negative_prompt) #return tensor of [uncond_embeddings, text_embeddings] shape = [4,77,768]
            added_cond_kwargs = {}
    ...
    seq_iter, seq_next_iter = rg_helpers.get_seq_iter(model.scheduler.timesteps, run_inversion) #return list and iterator of the steps from T to 0
